...
description: Agents that need to run Claude MCP cross-review must use this runtime skill to decide applicability, perform local preflight, and hand off execution to the Claude cross-review protocol.
---
...
## Scope
- You must apply this document when the task requires Claude MCP cross-review for code, document, configuration, or rule changes.
...
## Source of Truth
- This document governs Claude review runtime entry conditions, review-phase applicability, local preflight checks, and protocol handoff behavior in `./SKILL.md`; it does not govern Claude finding normalization, collaboration-log schema, or history-record creation criteria.
- `../tool-usage-management_claude-cross-review-protocol/SKILL.md` is the single source of truth for Claude invocation structure, mandatory review axes, finding normalization, and collaboration log format; consult it when deciding how a review invocation must be structured and reported after runtime entry succeeds, not for phase applicability or local preflight decisions.
- `../skill-management_skill-modify-history/SKILL.md` is the single source of truth for history-record obligations when revising this runtime skill or the protocol; consult it when deciding whether and how a revision must be recorded in history artifacts, not for Claude review startup behavior.
...
- Before invoking Claude MCP, you must run a minimal responsiveness check through the `nowonbun_claude` server that expects `OK` as the exact output.
- When the MCP availability check fails, you must stop Claude review and report `cross-review: blocked (reason: claude mcp unavailable)`.
- When the responsiveness check fails, you must stop Claude review and report `cross-review: blocked (reason: claude mcp unresponsive)`.
...
- You must keep the request body short enough to satisfy runtime request-size controls.
...
## Execution Rules
- After local preflight succeeds, you must follow `../tool-usage-management_claude-cross-review-protocol/SKILL.md` for the actual Claude invocation and reporting.
- You must use `mcp_servers.nowonbun_claude` as the execution entry point for this harness.
- When invocation failure occurs, you must follow the retry and blocked-state rules defined in `../tool-usage-management_claude-cross-review-protocol/SKILL.md`.
...
# Must NOT
- You must not redefine finding normalization rules that already belong to `../tool-usage-management_claude-cross-review-protocol/SKILL.md`.
- You must not redefine collaboration log format that already belongs to `../tool-usage-management_claude-cross-review-protocol/SKILL.md`.
...
5. Hand off execution to `../tool-usage-management_claude-cross-review-protocol/SKILL.md`.
...
- Invocation, normalization, and reporting rules were delegated to `../tool-usage-management_claude-cross-review-protocol/SKILL.md` without duplicate local rule text.
