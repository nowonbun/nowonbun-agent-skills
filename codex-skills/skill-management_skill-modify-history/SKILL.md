...
description: 기대치 불일치 후 기술 또는 관리 문서를 수정하는 담당자는 원인, 수정 사항 및 예방 조치에 대한 세부 정보를 확정적 이력 파일에 기록해야 합니다.
---
...
- 이 문서는 스킬 마크다운 형식 디자인이 아닌, 이력 기록 의무 및 생성 시점 기준에 적용해야 합니다.

## Source of Truth
- 이 문서는 이력 기록 생성 의무, 필수 콘텐츠 섹션 및 일별 생성 기준을 규정합니다. 스킬 문서 마크다운 구조, 서문 형식 또는 관련 아티팩트 요구 사항은 규정하지 않습니다.
...
- 규칙 문서를 편집하기 전에 일별 히스토리 생성이 필요한지 여부를 결정해야 합니다.
- 아래 트리거 중 하나 이상이 참일 경우 즉시 `history/skill_YYYYMMDD.md` 파일을 생성해야 합니다.
...
- 필수 히스토리 생성을 최종 다듬기 단계로 미루어서는 안 됩니다.
...
## Outputs
- daily-history 트리거 중 하나라도 참인 경우 `history/skill_YYYYMMDD.md` 파일이 생성됩니다.
...
- 트리거가 활성화된 날에 규칙 문서 수정을 완료하기 전에 `history/skill_YYYYMMDD.md` 파일을 생성해야 합니다.
...
3. 트리거가 참이면 `history/skill_YYYYMMDD.md` 파일을 즉시 생성하고, 그렇지 않으면 최종 실행 보고서에 `daily-history-trigger: no-op`을 기록합니다.
4. 관련 규칙 문서를 수정합니다.
5. 근본 원인, 변경 사항 및 예방 규칙을 기록 아티팩트에 기록합니다.
6. `## Cross-Review Requirement Rule`에 정의된 필수 교차 검토 단계를 실행하고 결과를 최종 실행 보고서에 기록합니다.
7. 완료 보고 전에 수정된 규칙 파일과 생성된 기록 파일을 교차 검증합니다.

# Definition of Done

## Verification
- 규칙 문서 편집 전에 `## Daily History Trigger Rules`의 검사가 완료되었습니다.
- 트리거 결과에 따라 `## Outputs`에 명시된 필수 출력물이 생성되었습니다.
